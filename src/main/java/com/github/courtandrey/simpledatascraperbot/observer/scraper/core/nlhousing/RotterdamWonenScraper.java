...
import com.github.courtandrey.simpledatascraperbot.observer.scraper.core.parser.nlhousing.RotterdamWonenParser;

import java.util.List;

public class RotterdamWonenScraper implements Scraper<RentalOffering> {
    private final RotterdamWonenParser parser = new RotterdamWonenParser();

    @Override
    public List<Pair<Request, Processee<RentalOffering>>> scrap(List<Request> reqs) {
        return new PageScrapingFunction<>(
                req -> new GETConnector("https://www.rotterdamwonen.nl/en/aanbod/page/%d"),
                parser::parsePage,
                NLHousingRequest.class
        )
                .sameOutput(true)
                .withFilteringCondition(new RentalOfferingRequestSatisfied())
                .withFilteringCondition((req, off) -> !off.getName().contains("Rented"))
                .apply(reqs);
    }
    ...
    public boolean rightScraperToRequest(Request request) {
        return request instanceof NLHousingRequest nlHousingRequest
                && nlHousingRequest.getSites().contains(NLHousingSite.ROTTERDAM_WONEN);
    }
}
