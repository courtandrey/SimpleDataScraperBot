...
import com.github.courtandrey.simpledatascraperbot.observer.scraper.core.parser.nlhousing.NationaalGrondbesitParser;

import java.util.List;

public class NationaalGrondbesitScraper implements Scraper<RentalOffering> {
    private final NationaalGrondbesitParser parser = new NationaalGrondbesitParser();

    @Override
    public List<Pair<Request, Processee<RentalOffering>>> scrap(List<Request> reqs) {
        return new PageScrapingFunction<>(
                req -> new GETConnector("https://www.nationaalgrondbezit.nl/huuraanbod"),
                parser::parsePage,
    ...
    public boolean rightScraperToRequest(Request request) {
        return request instanceof NLHousingRequest nlHousingRequest
                && nlHousingRequest.getSites().contains(NLHousingSite.NATIONAALGRONDBEZIT);
    }
}
