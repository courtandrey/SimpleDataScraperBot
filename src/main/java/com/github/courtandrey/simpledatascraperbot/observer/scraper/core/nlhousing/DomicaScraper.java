...
import com.github.courtandrey.simpledatascraperbot.observer.scraper.core.parser.nlhousing.DomicaParser;
...
public class DomicaScraper implements Scraper<RentalOffering> {
    private final DomicaParser parser = new DomicaParser();
    ...
    public boolean rightScraperToRequest(Request request) {
        return request instanceof NLHousingRequest nlHousingRequest
                && nlHousingRequest.getSites().contains(NLHousingSite.DOMICA);
    }

    private String getUrl(NLHousingRequest request) {
        String template = "https://www.domica.nl/rts/collections/public/3d202c54/runtime/collection/EAZLEE/data" +
                "?page=%7B%22pageSize%22%3A%228%22%2C%22pageNumber%22%3A$PAGE_NUM%7D" +
                ...
                "&sortBy=%7B%22field%22%3A%22ranking%22%2C%22direction%22%3A%22asc%22%7D&language=ENGLISH_UK";
