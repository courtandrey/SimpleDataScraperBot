...
import com.github.courtandrey.simpledatascraperbot.observer.scraper.core.parser.nlhousing.HurenbijwoooveParser;
import java.util.List;

public class HurenbijwoooveScraper implements Scraper<RentalOffering> {
    private final HurenbijwoooveParser parser = new HurenbijwoooveParser();

    @Override
    public List<Pair<Request, Processee<RentalOffering>>> scrap(List<Request> reqs) {
        return new PageScrapingFunction<>(
                req -> new GETConnector("https://hurenbijwooove.nl/Woning/Pagina/%d"),
                parser::parsePage,
    ...
    public boolean rightScraperToRequest(Request request) {
        return request instanceof NLHousingRequest nlHousingRequest
                && nlHousingRequest.getSites().contains(NLHousingSite.HURENBIJWOOVE);
    }
}
