...
public class RivaRentalsParser {
    public List<RentalOffering> parsePage(String docToParse) {
        Document document = Jsoup.parse(docToParse);
        Elements rentals = document.getElementsByClass("house-overview-single-house");
        ...
        for (Element rental : rentals) {
            ...
            rentalOffering.setCity(textFromElements(rental.getElementsByTag("h3")).split(",")[1].trim());
            rentalOffering.setPrice(textFromElements(rental.getElementsByClass("price")));
            rentalOffering.setUrl("https://www.rivarentals.com" + rental.getElementsByAttribute("href").attr("href"));
    ...
    public void addDetails(String docToParse, RentalOffering data) {
        ...
        for (Element tr : document.getElementsByTag("tr")) {
            ...
            if (tds.get(0).text().contains("Available")) {
