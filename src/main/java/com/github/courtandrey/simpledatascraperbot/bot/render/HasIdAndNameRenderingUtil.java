...
public final class HasIdAndNameRenderingUtil {
    private static final String SELECT_ALL_NAME = "All";
    ...
    public static <T extends HasIdAndName> String renderTextWithSelectAll(T[] hasIdAndNames) {
        return renderText(hasIdAndNames) + "\n"
                + String.format("%d. %s", getSelectAllId(hasIdAndNames), SELECT_ALL_NAME);
    }

    public static <T extends HasIdAndName> int getSelectAllId(T[] hasIdAndNames) {
        return getMax(hasIdAndNames) + 1;
    }

    public static <T extends HasIdAndName> boolean isValidOrSelectAll(T[] hasIdAndNames, int number) {
        return isValid(hasIdAndNames, number) || number == getSelectAllId(hasIdAndNames);
    }
